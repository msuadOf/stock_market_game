...
- 行情数据真实性：五档盘口只展示引擎订单簿中的真实档位；涨停无卖单、跌停无买单或流动性不足时保留空档，不伪造挂单。报价成交量、五档数量、逐笔成交、分时量、K 线量和资金成交统计均以一手 100 股换算；新游戏 NPC 委托必须为整手，旧存档零碎股按小数手精确显示，不能舍入成 0。委托输入与持仓仍以股计，防止仅为展示单位而改变交易契约。连续竞价期间每个权威 `PriceTick` 同时刷新最新价、买卖前五档和买一/卖一，禁止盘口停留在首次连接快照。盘口量色块按卖盘、买盘各自五档中的最大挂单量归一，卖盘淡绿、买盘淡红，并保持右对齐和数字可读。
...
- 集合竞价：默认交易日先走 09:15–09:30 共 900 tick 的集合竞价，再走 240 分钟连续竞价。竞价指示价、虚拟成交量和 09:30 开盘结果只取 Rust 引擎事件；无交叉委托时显示空状态，不以前端估值代替。分时纵轴同时覆盖竞价与盘中全部有效点，以昨收为中心取最大绝对偏离，上下价格边界与右侧正负涨跌幅严格对称。
