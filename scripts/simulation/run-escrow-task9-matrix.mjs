...
function negativeControlDetected(capture) {
  return capture.public_payload_order_probe?.probe_disabled_sort_changed_bytes === true
    || capture.scheduler_precanonical_order?.probe_disabled_sort_changed_bytes === true
    || capture.scheduler_precanonical_order?.disabled_merge_changed_bytes === true
    || capture.negative_control?.detected === true;
}
...
function harnessInvocation(entry, output) {
  ...
  return {
    command: "cargo",
    args: ["run", "--quiet", "-p", "engine", "--example", "escrow_verification_harness", "--", ...harnessArgs],
    printable: ["cargo", "run", "--quiet", "-p", "engine", "--example", "escrow_verification_harness", "--", ...harnessArgs],
  };
}
...
async function validateStoredPass(config, request, requestHash) {
  let summary;
  try {
...
export async function runTask9Matrix(inputConfig, { runChild = defaultRunChild } = {}) {
  ...
  if (await pathExists(config.outputRoot)) {
    try {
      ...
      return await validateStoredPass(config, request, requestHash);
    } catch (error) {
      return {
        schema: SCHEMA,
        status: "FAIL",
...
async function frozenSourceFingerprint(sourceRoot) {
  const status = await runGit(sourceRoot, ["status", "--porcelain", "--untracked-files=all"]);
  if (status !== "") {
    throw new MatrixFailure("SOURCE_NOT_FROZEN", "Task 9 matrix requires a clean frozen source checkout", { status });
  }
  const revision = await runGit(sourceRoot, ["rev-parse", "HEAD"]);
  return sha256Hex(Buffer.from(`git-commit:${revision}\n`));
}
