 ...
 * one runnable-thread sample. CPU utilization is intentionally not collected
 * or substituted for throughput.
 */
...
const REPORT_SCHEMA = "escrow-perf-report-v2";
...
export async function runPerformanceHarness(config, { runSample = runProcessSample, now = () => new Date().toISOString(), environment = runtimeEnvironment } = {}) {
  ...
  for (let index = 0; index < config.warmup_runs; index += 1) {
    for (const side of ["before", "after"]) {
      const measured = await runSample(config[side], { rssSampleIntervalMs: config.rss_sample_interval_ms, warmup: true, side, index });
      validateMeasuredSample(measured, config[side], config.workload, `${side} warmup ${index}`, side === "after", config.environment_contract);
    }
  }
...
export async function main(argv, { runHarness = runPerformanceHarness, environment = runtimeEnvironment, stdout = console.log } = {}) {
  ...
  const report = await runHarness(config, { environment: () => currentEnvironmentManifest });
