...
import type { StrategyProfile } from "../../types/generated/StrategyProfile"
import { array, decimal, exact, integer, map, oneOf, record, string } from "./primitives.ts"
...
function accountKey(value: string, path: string): void {
  if (!/^\d+$/.test(value)) throw new Error(`存档 ${path} 账户键无效`)
}
...
export function parseOrderState(value: Record<string, unknown>): Pick<import("../../types/engine").SaveSlot, "auction_orders" | "resting_orders" | "price_history" | "market_minute_closes" | "rng_state" | "npc_attention" | "strategy_profiles" | "parent_orders" | "npc_order_lifecycles" | "pending_player" | "next_order_id"> {
  const auction_orders = map(value.auction_orders, "auction_orders", stockCode, (items, path) => array(items, path).map((item, index) => { const order = record(item, `${path}[${index}]`); exact(order, ["owner", "side", "limit", "qty", "arrival_seq"], `${path}[${index}]`); return { owner: integer(order.owner, `${path}[${index}].owner`, 0), side: oneOf(order.side, `${path}[${index}].side`, side), limit: money(order.limit, `${path}[${index}].limit`), qty: integer(order.qty, `${path}[${index}].qty`, 1), arrival_seq: integer(order.arrival_seq, `${path}[${index}].arrival_seq`, 0) } }))
  ...
  const strategy_profiles = map(value.strategy_profiles, "strategy_profiles", accountKey, (item, path): StrategyProfile => { const profile = record(item, path); const keys = Object.keys(profile); if (keys.length !== 1) throw new Error(`存档 ${path} 必须是单一策略变体`); switch (keys[0]) { case "Retail": return { Retail: oneOf(profile.Retail, `${path}.Retail`, ["Dormant", "LongTerm", "Noise", "DipBuyer", "Momentum", "Panic"] as const) }; case "Institution": return { Institution: oneOf(profile.Institution, `${path}.Institution`, ["DeepValue", "Growth", "Balanced", "Defensive", "ActiveTrader"] as const) }; case "Hot": return { Hot: oneOf(profile.Hot, `${path}.Hot`, ["Momentum", "Reversal"] as const) }; default: throw new Error(`存档 ${path} 包含无效策略变体`) } })
  ...
  return { auction_orders, resting_orders, price_history, market_minute_closes, rng_state: decimal(value.rng_state, "rng_state"), npc_attention, strategy_profiles, parent_orders, npc_order_lifecycles, pending_player, next_order_id: integer(value.next_order_id, "next_order_id", 1) }
}
