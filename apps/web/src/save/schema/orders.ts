...
import { array, decimal, exact, integer, map, oneOf, record, safeIntegerKey, string } from "./primitives.ts"
...
export function parseOrderState(value: Record<string, unknown>): Pick<import("../../types/engine").SaveSlot, "auction_orders" | "resting_orders" | "price_history" | "market_minute_closes" | "rng_state" | "npc_attention" | "parent_orders" | "npc_order_lifecycles" | "pending_player" | "next_order_id"> {
  ...
  return { auction_orders, resting_orders, price_history, market_minute_closes, rng_state: decimal(value.rng_state, "rng_state"), npc_attention, parent_orders, npc_order_lifecycles, pending_player, next_order_id: integer(value.next_order_id, "next_order_id", 1) }
}
