import { array, boolean, exact, integer, map, nullable, oneOf, record } from "../primitives.ts"
import { externalTag, money, orderId, planKey, stockKey, tagged, type StringMap } from "./common.ts"
...
export type ReviewConditions = { readonly min_signal_delta_bp: number; readonly min_price_change_bp: number; readonly last_review_signal_score_bp: number; readonly last_review_trading_day: number; readonly last_review_price: number | null; readonly last_review_acquired_count: number }
...
function parseReview(value: unknown, path: string): ReviewConditions { const parsed = record(value, path); exact(parsed, ["min_signal_delta_bp", "min_price_change_bp", "last_review_signal_score_bp", "last_review_trading_day", "last_review_price", "last_review_acquired_count"], path); return { min_signal_delta_bp: integer(parsed.min_signal_delta_bp, `${path}.min_signal_delta_bp`), min_price_change_bp: integer(parsed.min_price_change_bp, `${path}.min_price_change_bp`), last_review_signal_score_bp: integer(parsed.last_review_signal_score_bp, `${path}.last_review_signal_score_bp`), last_review_trading_day: integer(parsed.last_review_trading_day, `${path}.last_review_trading_day`, 0), last_review_price: nullable(parsed.last_review_price, `${path}.last_review_price`, money), last_review_acquired_count: integer(parsed.last_review_acquired_count, `${path}.last_review_acquired_count`, 0) } }
