...
describe("authoritative K-line event sync", () => {
  it("uses Rust candles across a boundary without rebuilding an old event on the new day", () => {
    ...
    const result = reduceCandleEvents(completed, active, [
      { PriceTick: { seq: 1, tick: 1, code: "600101", last_price: 1100, daily_candle: { time: 0, open: 1050, high: 1200, low: 1000, close: 1100, volume: 30 } } },
      { DayBoundary: { seq: 2, day: 1, closed_daily_candles: { "600101": { time: 0, open: 1050, high: 1200, low: 1000, close: 1100, volume: 30 } } } },
      { PriceTick: { seq: 3, tick: 14_401, code: "600101", last_price: 1220, daily_candle: { time: 86400, open: 1200, high: 1250, low: 1180, close: 1220, volume: 4 } } },
    ]);
  ...
  it("keeps only the last authoritative active candle per stock in a high-speed batch", () => {
    ...
    const result = reduceCandleEvents({}, {}, [
      { PriceTick: { seq: 1, tick: 1, code: "600101", last_price: 1000, daily_candle: { time: 0, open: 1000, high: 1000, low: 1000, close: 1000, volume: 0 } } },
      { PriceTick: { seq: 2, tick: 2, code: "600101", last_price: 1100, daily_candle: { time: 0, open: 1000, high: 1200, low: 900, close: 1100, volume: 20 } } },
    ]);
