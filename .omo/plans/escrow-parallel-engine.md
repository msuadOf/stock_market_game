...
Your next move: 由协调者核对已有成果与全部后代的在途工作，按模块批次登记接口版本、隔离环境和验收场景；并行启动连续交易、竞价、存档、宿主和工具的就绪实现，main 持续集成，不重做已验收组件或等待单个会话包办全部工作。
...
## Execution strategy
> 2026-09-20 全量重排版：执行单位改为“完整模块/联合交付”，采用先成组编码、后集中补测/审核/调试、通过即本地提交的流水线。此节完整替代旧 Wave 串行开工安排；任务编号 1–12 与 F1–F4 保留作验收索引，checkbox 和历史证据不重置。运行时阶段契约同时按下节及 ADR-0017 修订；开发批次不是游戏 TickBatch。
...
### Module batches and dependency matrix

“接口已固定”即可开始模块真实实现，不要求上游 checkbox 或独立审核先完成。缺失实现可形成明确编译缺口，不能冒充通过。下表替代旧任务 7 完成后才可写存档、任务 9 完成后才可写工具的开发限制；最终验收仍检查实际依赖及全部原场景。

| 批次 | 一次写齐的交付范围 | 开工条件 / 原任务映射 | 最终验收与提交边界 |
| --- | --- | --- | --- |
| B1 连续交易与自适应计划链 | P2 三来源、持续 P3 预算/ID、增量 P4、私有 continuation 投影、P5–P9 整体闭环 | 已封存基线 + 本版运行时契约；任务 3–7 对应部分 | 真实多根计划与成交/撤单/跨 tick/失败完整场景；语法或玩家单一路径不能代表整批完成 |
| B2 竞价与日界 | 竞价接入、余单、日终收尾、跨源收据、边界回滚 | 共享操作/收据接口固定即可与 B1 并行；任务 3/4/5/6/7 | 与实际 B1 合并验证、原竞价规则与收尾一次性；存档子场景由 B3 回填 |
| B3 存档与恢复 | schema v2、全状态编解码、订单/账本/游标/策略/seen 校验、恢复续跑 | 字段及静默点契约固定即可编码，不等待任务 7；任务 8 | 真实 B1/B2 非空状态往返及原 save_contract；回填任务 3/6 存档证据，无正式启用前置 |
| B4 三宿主与失败交付 | web-wasm/server/desktop 错误映射、TS 契约、完整帧与 CivilUpdate 接缝 | 公共 Result/HostFailure/更新协议固定即可编码；任务 7 | 隔离真实引擎接入与三宿主测试；可与 B3 并行，不等存档实现才验收 fatal 映射 |
| B5 验证及诊断工具 | 确定性/扰动/守恒/语料比较器、性能 harness、诊断审计、K7 核验脚本 | 数据格式/来源契约固定即可写工具和负测试；任务 9/10/11 | 工具模块可先独立验收提交；最终数值和诊断期望只能依完整新引擎证据，不预填 PASS |
| B6 文档与最终证据 | 文档符号检查、架构/规则说明、证据索引、最终矩阵及 F1–F4 | 工具/说明可持续准备；任务 12/F1–F4 | 最终确定性/性能待 3–8 全验收；诊断期望待任务 9 证据；K7 待诊断及核验工具纳入稳定源码；终稿只写已验证结论 |

依赖分三种：**开发**只依赖明确接口/文件归属；**候选集成**可使用已登记的未审依赖修订，并将接口失败汇入联合调试；**验收/提交**要求实际依赖闭包已审已测。跨批候选依赖登记精确指纹和失效传播，不能冒充已验收基线；不得为了提前开发把所有 B1–B6 合成一个无限增长的大包。每条线最多 2 个未集成验收批次，读取一个固定的上游候选版本不额外创建上游 WIP，但本线自己的消费者批次照常计数。

任务索引的最终门禁：1/2 的历史成果复用；3/6 的运行时验收可先交付，存档子验收等 B3 回填；4/5 按真实接入验收；7 按完整运行时失败与 B4 三宿主验收，不等 3/6 的存档子验收；8 按 B3 实际非空状态验收，不等 7 checkbox；9 的最终矩阵等 3–8 完整验收；10 的最终诊断迁移依 9；11 最终 K7 依 10 和核验脚本的稳定指纹；12 终稿依最终证据，符号检查器等代码改动应先完成并纳入最终源码；F1–F4 在全部任务完成后独立出结论。不存在“开发工具必须等最终引擎完成”的前置。

### Seats, models and owned modules

默认总并发上限 12，包含 main、全部子孙、8 个实现席位、2 个独立审查席位和 V1；平台可用槽更少时缩减。席位是可复用容量，不是固定阶段岗位。以下模型/强度是用户授权的请求目标，使用当前工具实际提供的 ID；fast 不是 reasoning effort，不对价格/服务端采用情况作未经核实的承诺。

| 席位 | 请求模型 / 强度 | 当前整模块分工及下一项 |
| --- | --- | --- |
| Main 集成负责人 | Sol `high`；仅调度可 `medium` | 契约、唯一候选组装、共享入口/模块注册、跨批集成、验收与串行 commit |
| L1 | Sol `high`；RNG 边界 `xhigh` | NPC/玩家候选及顺序，交付后转独立场景/宿主适配 |
| L2 | Astra `xhigh` 或 Sol `xhigh` | 完整自适应计划链、continuation、私有执行事实投影；不得交付 single-yield 作为完整实现 |
| L3 | Sol `high`；溢出/预算 `xhigh` | 持续 P3 预算/ID/约束状态与跨轮测试 |
| L4 | Sol `high`；跨源因果 `xhigh` | 连续 P4 及 P5/P6 接缝缺口，优先复用已交付账本/结算 |
| L5 | Sol `xhigh` | B2 整体竞价/日界/收尾模块 |
| L6 | Sol `high`；恢复一致性 `xhigh` | B3 完整存档/恢复模块；维护所需保留测试映射 |
| L7 | Sol `high` | B4 三宿主适配；各宿主可在总席位内拆给其他空闲实现者 |
| L8 | Sol `high`；确定性设计 `xhigh` | B5 工具及集中测试代码；缺口定位后交对应实现属主修业务逻辑 |
| Reviewer A | Astra `xhigh` | 完整批次的语义/原子性/依赖审查；参与设计或实现则该批回避 |
| Reviewer B | Sol `high`，跨层 `xhigh` | 独立完整 diff 复核、必要性/边界；可审另一批，至少一人承担每批完整审查 |
| V1 | Terra `medium`；构建诊断 `high` | 全量构建、完整回归/矩阵、重型作业资源和缓存；不改业务代码 |

Codex 原生 ID 对应 `gpt-5.6-sol`、`gpt-6-astra`、`gpt-5.6-terra`、`gpt-5.6-luna`，执行时以工具 schema 为准。Luna `low` 仅空闲席位做索引/证据整理，`medium` 可做已定场景的机械夹具；不能替代高风险实现或唯一语义审查。Terra 可在空闲实现席位处理机械适配与文档，核心编码默认 Sol/Astra。记录请求与实际可观测模型/强度，无法确认服务端采用则明示；不静默降级，不为改模型覆盖在途工作。当前 main 无法切换时记录差异，将复杂方案检查交合适独立会话，main 保留集成归属；不能仅凭计划文本宣称会话已升档。

文件归属按具体模块登记，不按旧 L 编号推断；原 L1–L8 交付先映射到新表并复用，已完成者转下一完整模块。`session.rs`、`pipeline/mod.rs`、`pipeline/shadow.rs` 及共享类型/入口只能由 main 或 main 明确交接的唯一集成属主写；worker 交稳定补丁，不互抢路径。子 agent 可在 main 分配的空闲额度和明确文件边界内派孙 agent，计入总上限；不得递归无上限扩张，审查独立性同样适用。
...
批次按完整能力边界定范围，不以行数凑量，不拆成逐 wrapper 审查，也不把整个项目一次堆完。一个模块冻结后不得不断追加新功能；初审/验证期间继续下一已就绪模块。存在未决计划链分支时，可以冻结当前修订供诊断审查、验证其他场景，但不能将其标为 B1 完整验收通过或提前启用。
...
等待前必须检查候选接线、固定接口下游编码、测试夹具、修复、提交、空闲席位就绪任务与验证请求。连续两次同因等待无进展要看实际作业输出和依赖，不能只 `list_agents`。一次定位加一次修复仍不解的具体问题交 Astra `xhigh` 有界分析，再由实现者一次修完整边界；设计咨询者回避该批最终独立审核。worker/reviewer 交付后无已分配工作则结束本轮，main 续派；不让已交付者空占席位。

**并行度要求**：接口/文件/资源允许时，优先让 B1 关键路径、B2、B3、B4/B5 至少形成多个独立工作面；若活跃实现只剩 main，应逐项说明哪些模块已完成、受何具体依赖/WIP约束或为何不能派发，不能笼统说“等 B1”。不硬凑 12 个 running，不用重复测试或新研究任务装饰并行度。按实际编码/补测/审核/验证/等待/已结束分类汇报全部后代；编译期间模型分析可重叠，重型作业数量另受 V1 限制。
...
- **源码和构建隔离**：worker 在独立源码副本/worktree 开发；main 从稳定交接组装一个当前候选修订。可以有多个不同批次副本，禁止多个作者并写同一候选路径。副本包含必要的未提交/未跟踪依赖，不仅复制 HEAD。A 的审核/测试用不可变完整输入，B 的写入在另一副本；不能边改 A 边跑 A 的测试。无法隔离时登记单写/冻结窗口，其他不冲突工作继续。
...
- **正式启用**：候选可替换 legacy bridge 做真实全链验证，但不能同一操作走两遍。用户路径的正式切换须新权威状态持久化/恢复、全部运行时和宿主门禁以及最终相关验证齐备；B3 实现/验收不依赖正式启用。无关模块可先提交；不新增长期双业务实现，不提前把无法恢复的新状态交用户。
...
**#9 决策记录（替代批准门禁）**：卖单费用封顶语义由用户于 2026-09-17 在规划会话中明示裁定（"印花税按比例扣……手续费固定的 5 块钱，要是扣完不够了，就扣完扣到 0 就好"），本计划以此为准；无实施中人工批准门禁；ADR-0017 直接以 `Status: accepted` 落地并引用本决策记录。
  ...
  Parallelization: 契约文档维护；既有证据保留，本版运行时修订须独立复核，不重采历史基线；详见 Module batches and dependency matrix。
...
- [x] 2. 阶段化 tick 管线（全 shadow + 双账本 + 单点提交 + 内建并行）、step() Result 化与重构前基线采集
  ...
  Parallelization: 已封存基线/骨架复用；本版数据流契约供 B1–B5 并行开工，新增运行时行为由对应批次补验收；详见 Module batches and dependency matrix。
...
- [ ] 4. 校验拆分：账户阶段 vs 股票处理阶段
  ...
  Parallelization: B1/B2 持续账户校验与增量股票处理；接口固定即可并行，验收含初轮及后轮溢出整 tick 回滚；详见 Module batches and dependency matrix。
...
- [ ] 5. 收据驱动结算与分配截点语义
  ...
  Parallelization: B1/B2 收据结算；接口固定即可并行，完整流后一次 P6，实际接入后验收；详见 Module batches and dependency matrix。
...
- [ ] 6. 股票状态机：密封序、撤单/替换与集合竞价
  ...
  Parallelization: B2 竞价/日界及 B1 连续状态机；与 B1/B3 并行，最终验收含真实接入及 B3 恢复子场景；详见 Module batches and dependency matrix。
...
- [ ] 7. 失败模型（全 shadow + 单点提交 + 类型化毒化）与三宿主接入
  ...
  Parallelization: B1/B2 失败模型 + B4 三宿主；接口固定即可开发，完整运行时/宿主证据后验收，不等待 3/6 存档子验收，也不阻塞 B3 开工；详见 Module batches and dependency matrix。
...
- [ ] 8. 存档 v2 与静默点契约
  ...
  Parallelization: B3；持久化字段/静默点契约固定即可与 3–7 并行真实实现；验收依真实新状态往返和续跑，回填 3/6，最终任务 9 前完成；详见 Module batches and dependency matrix。
...
- [ ] 9. 确定性三重门禁 + 守恒 + 性能证据 harness
  ...
  Parallelization: B5 工具提前并行实现/自测/独立提交；完整确定性/语料/性能验收依 3–8 全部证据；详见 Module batches and dependency matrix。
...
- [ ] 10. 诊断再基线
  ...
  Parallelization: B5 诊断审计工具提前并行；最终期望迁移依任务 9 证据，完成后放行 11 最终矩阵；详见 Module batches and dependency matrix。
...
- [ ] 11. 全新 K7 矩阵重跑（包含链契约 + 独立核验脚本）
  ...
  Parallelization: B5 核验脚本提前并行实现和负测试；最终 94 次 K7 依 10 及所有相关工具代码纳入稳定源码指纹；详见 Module batches and dependency matrix。
...
- [ ] 12. 文档最终同步
  ...
  Parallelization: B6 文档和符号检查工具提前准备；代码工具纳入最终指纹后跑矩阵，终稿依 9–11 最终证据；随后 F1–F4；详见 Module batches and dependency matrix。
