...
- **并发验收与资源排程**：B1–B5 的独立冻结修订可以同时审核和验证，无真实依赖的验收不等待 B1 或其他整批完成。V1 是统一资源/全量作业负责人，不是所有批次串行验收的锁；V1 可启动多个后台作业并收取结果。首次无资源观测时以最多 2 个重型作业、其中最多 1 个全量构建为起点；V1 必须依据工作区可用空间、CPU、内存和实际峰值调整并发及各作业 Cargo jobs，资源允许时提高并发（包括独立全量作业），不能把初始值作为永久上限。轻量检查/独立审核不因重型队列停工；性能测量按其要求独占以避免干扰。实际依赖、同一可写缓存、源码冻结窗口冲突才限制对应作业。
