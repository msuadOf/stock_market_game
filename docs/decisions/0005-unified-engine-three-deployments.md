...
| 通道 | 用途 | 协议 |
|------|------|------|
| **每客户端 Publisher** | 行情流、成交事件、状态变更，以及客户端写请求入队 | **`wss://`** WebSocket 持久全双工；每客户端可选服务端 16ms 定时推送或客户端 `GetFrame` 拉取（本阶段 JSON 载荷） |
| **请求** | 下单/撤单/查询/登录/存档 | **HTTPS REST** 请求/响应 |

**公网 WS 韧性契约**（公网长连接必踩，从设计内建而非事后补丁）：
- actor 内部生成 `EngineUpdate { events, runtime_snapshot? }` 原子批次；每个 WS 客户端的
  Publisher 将这些批次压缩并编码为
  `PublisherFrame { from_seq, to_seq, events, runtime_snapshot? }`。覆盖区间明确记录原始事件水位，
  因而允许同一行情槽覆盖旧采样而不会被误判为丢包；每个集合竞价 6 秒槽、每个连续竞价分钟槽、
  日界及委托/错误事件必须保留；可见成交带与三宿主统一只保留最新 100 条，不作为完整逐笔历史。
