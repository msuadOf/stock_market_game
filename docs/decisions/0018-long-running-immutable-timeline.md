...
- **状态：** proposed
- **日期：** 2026-09-23
- **决策者：** msuad（目标与观察语义已提出；本文列出的冲突项仍待逐项确认）
...
计算第 `n` 个 tick 的策略决定时，玩家和 NPC 应看到同一个已经提交的公开市场版本，其行情
截止第 `n-1` 个 tick。这个版本一旦被选定，在本次决策期间不能被后续撮合改写。比如第 101
个 tick 中，NPC A 的买单先成交，使股价从 10 元变为 10.10 元；NPC B 这次决策仍应看到
`S[100]` 的 10 元，不能因为它的计算线程晚启动就看到 10.10 元。玩家从页面提交请求时，也
必须标明页面实际展示的是哪个已提交版本，不能把旧页面的报价冒充为刚发生的价格。
...
再独自归并、排序、计算全局哈希。等待所有任务完成称为阶段屏障。如果并行阶段只有几项
很短的工作，而屏障两侧的串行工作很重，增加可用 CPU 数也不会显著加速。

以下是已经在代码中找到的具体增长路径；各条可能同时发生：

- [`clone_for_tick_shadow`](../../packages/engine/src/session.rs#L1287) 深复制 markets、全部账户、
  行情历史、公司状态、计划和账本；决策捕获又在
  [`capture_decision_snapshot`](../../packages/engine/src/session/pipeline/decision_snapshot_capture.rs#L80)
  中建立嵌套 session clone。这里的 shadow 是一份失败时可以丢弃的临时世界。假如 30 年里
  已积累大量公司分录和终止计划，即使本 tick 没有碰这些旧数据，整局复制仍要经过它们。
- [`StateHash::field`](../../packages/engine/src/session/hash.rs#L15) 先把字段完整 JSON 序列化，
  `business_state_hash` 又覆盖全部日 K、计划、账户和其他集合。健康 tick 还会多次计算全状态
  rollback hash。哈希可以理解成一段数据的校验指纹；当前做法要先把老数据再次写成 JSON
  字节，再逐字节计算指纹。它能检查状态，却让普通 tick 的计算量跟旧状态总量一起增长。
- 完成日 K 目前只保留 360 根，超出后直接删除，见
  [`commit_active_daily_candles`](../../packages/engine/src/session/candles.rs#L57)。普通策略视图则把
  tick 价格和当日分钟价格复制成 owned `Vec`，且没有暴露完整日 K，见
  [`build_market_view`](../../packages/engine/src/session/views.rs#L12)。因此现有有限数组看起来控制住
  了长度，但它已经丢掉用户要求的完整日 K；而把数组扩大到几十年，又会让上面的全量复制和
  哈希更慢。需要同时改保存方式和热路径，不能只把 `360` 改成更大的数字。
- [`PlanBook`](../../packages/engine/src/plans/mod.rs#L128) 将终止计划永久留在热 `BTreeMap`，序列化时
  clone 全部计划；部分计划执行和日终路径还会扫描全簿。存档的百万计划上限只是校验门槛，
  不是运行时回收策略。假设某账户每年结束一条旧计划并建立一条新计划，30 年后真正活跃的
  可能只有一条，但每日扫描和存档会继续碰到早已结束的 30 条。
- 公司经营 Journal/posted batches、结账版本和重述、公开报告/公告库都会随经营年限增长；虽然部分
  ...
  但没有理由在每次股票报价变化时重新复制。
- `RetailProjectionSeen.receipts` 保存累计回执身份并在 P6 扫描；每个 NPC 的已获知公告记录、散户
  failure/exit experience 也会追加增长。这些是权威或防重语义，但不能继续作为每 tick 全量复制、
  全量扫描的热集合。回执用于证明一笔成交结算过、不能重做；NPC 的获知记录用于证明它当时
  知道什么。两类资料需要保留可核对的证据，但平常只需要快速判断本 tick 的新记录及当前状态。
- `TickFrame`/协议 checkpoint、诊断 causal facts、回执和事件等增长数据仍存在全量 clone、save、
  sort 或 hash 路径。开启 `simulation-diagnostics` 时，无界 causal facts 的累计分析甚至可能接近
  随事实数二次增长。`TickFrame` 是传给宿主/前端的一次 tick 完整结果，checkpoint 是出错
  或恢复时使用的保存点。诊断记录属于可选功能；即使开启，也不能把旧诊断每 tick 重算一遍。
...
- 当前生产 tick 未发现跨实体 `Mutex/RwLock/channel/actor` 争用。低并行度主要来自阶段
  `collect` 栅栏、串行准备/归并、逐命令 P3→P4 的“启动任务、等待结果、再启动下一条”循环，
  以及全量复制和哈希。某个计划一次只送一张订单进入按股票并行的撮合函数时，该轮其实
  至多只有一只股票有活可做。
...
本文所说“tick `n` 决策只看 `S[n-1]`”，精确含义是：在 `Freeze(n)` 前选择计算
tick `n` 所依据的最后一个**已提交**观察版本；它的市场行情最多到 `n-1`，并包含
该版本以前已经完成的 Civil 更新。例如前一天收盘的 market tick 是 500，夜间发布的
公告已作为 Civil 更新提交，那么次日第 501 个 market tick 的策略可以看见该公告；
不能机械地固定为“第 500 个市场 TickBatch 刚结束的那一刻”而漏掉合法披露。
本 tick 自己的 P0、撮合、结算和披露不能悄悄进入这个观察版本。观察令牌应标识完整
...
| P2 | NPC、玩家预置请求和计划根据授权观察版本形成普通意图；依赖型计划还可启动既定操作链。 | 普通投资决定只读旧 `ObservationRoot`；机械续执行的窄例外见第 6.2 节。 |
| P3 | 按账户和确定的顺序检查资源与申报条件，为通过的下单建立预留草案并分配必要身份。 | 账户 A 对两只股票同时买入，不能让先抢到锁的线程任意花掉同一笔钱。 |
...
| P8 | 检查待提交权威状态与摘要、回滚条件是否一致。 | 失败则丢弃整轮工作区。 |
...
是本轮成功后形成的新版本；`n-1` 和 `n` 是前后两个市场 tick。NPC 在本轮严格
使用 `v`；异步玩家可能先前看到另一个已提交版本 `u`，它可以早于 `v`，是否接受
...
flowchart LR
    ...
    HUMAN --> ADMIT["外部请求正式验收"]
    ADMIT --> FREEZE["Freeze n：固定本轮外部输入<br/>旧观察根及根计划资格"]
    R --> FREEZE
    FREEZE --> NPC
    R --> P0["P0 到期处理<br/>ExecutionBudget n"]
    NPC --> SEAL["固定输入导出的<br/>首轮就绪意图"]
    FREEZE --> SEAL
    P0 --> VALIDATE["按账户校验与确定性仲裁"]
    SEAL --> VALIDATE
...
1. 引擎固定本轮 NPC 和计划策略的旧观察版本；玩家提交的是其页面实际显示的已提交
   版本，它可能更早。两者都只能按自身权限查询公开历史及各自账户。若策略需要旧
   历史段，先按该版本加载并验证，不能在策略运行到一半时换成更新的历史。
2. P0 在私有执行分支处理已经过期的报价，P1 固定本轮可用于账户校验的资源截点。
3. `Freeze(n)` 先固定本轮可接收的**外部**玩家请求、观察根和根计划资格；之后才到达
   的外部请求不得插入本轮。NPC 根据这份固定旧根算出首轮意图，计划也从已经固定的
   根资格开始。由这些固定输入在本轮内部产生的后续命令，仍可以按已定义的因果关系
   加入本轮操作流；它们不是迟到的外部请求。
...
就需要为 B 所在账户页和 Y 所在股票页建立新内容，并更新从根找到这两页的少量目录节点；
A、C、X 的页和既有封存历史段继续共享。若一页包含多个账户，复制单位是页，不是单个
账户；页大小必须经基准测试选择，不能声称任何修改完全不复制。正在读版本 99 的人继续
...
当前存储里，完成日 K 只保留最近 360 根，这与用户希望多年后仍可查看完整已承诺历史冲突。
...
| 当前订单簿、仍有效的委托和其现金/股票预留、尚未收盘的活动日 K | 必须留在当前权威状态，因为本 tick 的校验、撮合和日终需要它们。 | 订单结束或日 K 收盘后，把结果及已承诺的生命周期事实归档；从当前订单簿移除，避免每 tick 再检查已经结束的委托。业务容量上限另行定义。 |
...
`Freeze(n)` 是“本轮的外部输入和根资格到此为止”的边界。在它之前被系统正式
验收入队、并被分配给本轮的**外部玩家请求**，加上固定的观察根和可启动的根计划
资格，构成本轮密封输入。这里把外部请求集合记作 `B_external[n]`；它不包含 P2
之后才由 NPC 在固定根上计算出的意图，也不包含本计划依据自身上一操作结果产生的
后续子命令。后两者可以在本轮内部按已冻结根、计划身份及确定性序号增量进入操作流，
但不能借此读取本轮新市场状态或接纳迟到的外部请求。客户端刚点击按钮、字节还在
...
“确定性”在这里是可重复计算，不是说每个 NPC 都作相同决定。给同一引擎版本、同一开局
状态、同一随机数种子（`seed`）、同一批已验收玩家命令及同一事先选定的业务顺序，
一台机器用 1 条计算线程、另一台用 8 条计算线程，最后的股票价格、成交、账户余额、
订单身份、计划状态和历史事件应相同。操作系统安排哪个任务先在 CPU 上运行，可以改变
物理完成时间，不能偷偷改变市场结果。这也是为什么不能用“谁先拿到一把锁，谁先花钱”
作为游戏规则。

假设账户 A 同时要以限价买入股票 X 和 Y，两单各需 8,000 元，而本轮 P1 可用现金
只有 10,000 元。X 与 Y 的股票订单簿本来可以独立计算，但账户 A 无法为两单同时
预留 16,000 元。必须先按确定的业务到达键决定哪张单优先获得预算，另一张按适用
...
当前管线存在 `npc → player → plan_chain` 的来源类别顺序。它可能把“是 NPC 还是玩家”
变成同批处理的隐含优先权，与统一交易者目标冲突。要去掉来源优先，必须先确定尚未获得
权威时间身份的**真正同时输入**如何进入订单簿。这不是可以随意在已验收订单上洗牌的
权限：已经有明确时间先后的订单，不能事后随机置换以破坏价格—时间优先。候选政策为：

| 候选到达政策 | 它具体怎样定先后 | 市场结果和重放的后果 |
| --- | --- | --- |
| 按持久化验收顺序 | 哪条请求先被正式登记，就先获得同批逻辑到达身份。 | 直观、易审计，但网络延迟和接入路径可能影响玩家顺序；NPC 与玩家的统一验收边界需明确定义。 |
| 按事先固定的交易者/请求键 | 对没有自然先后的同批请求按统一编号排序。 | 跨机器稳定，但较小编号可能长期有优势；不能把来源类别藏进编号。 |
| 对真正同时、尚未获得权威顺序的请求做可重放置换 | 在密封前固定输入集合与种子，然后产生确定的同批顺序并记录结果。 | 可能减少固定编号偏向，但会改变市场路径，须证明不可被操纵；若与现实受理时间不一致，应明确登记为游戏简化，绝不可重排已先后受理的订单。 |

这些只是供第 11 节裁决的方案，不是现有 A 股规则的替代品。若采用与现行交易所
真实受理时间不同的同批排序，必须在 [交易规则文档](../trading-rules.md) 说明其模拟
边界，并依据适用官方规则核对；不能把它无声写进性能重构。无论选择哪种方案，一旦
订单进入同一股票的订单簿，价格—时间优先及适用交易阶段仍是必须保持的约束。

并行实现中，每项任务只写自己负责的股票临时状态、账户变更记录或输出列表，不直接
共同修改一个可变全局订单簿。`canonical key` 是提交时用来稳定合并结果的规则键，
例如明确的阶段、实体编号和本实体内顺序；它不应被错误解释成跨股票真实成交先后。
`barrier`（屏障）是确有依赖的步骤必须等待，例如本轮请求先密封、同账户资源先仲裁、
全部适用撮合与计划链结束后才能统一结算和发布。优化目标是缩短不必要的屏障两侧串行
复制、逐候选 P3→单操作 P4 的反复启动等待、P6 单线程全量扫描及全局哈希，
...
独立账户同时观察、许多股票同时有订单，才有更宽的并行空间；此时也要以 tick
耗时、每秒 tick 数、串行尾部和相同输入下结果一致来验收，而不是把“占满 128 线程”
当作必须达到的正确性指标。
...
它继续计算。对旧档尤其要诚实：当前超过 360 根的早期日 K 已被旧实现删除，迁移
程序无法凭空找回；若接受旧档，必须标明对应历史流的最早可用范围与缺口，查询时
显式返回 `Unavailable`。若产品要求从 Genesis 起完整，只能从拥有完整源记录的新局/
新档开始，或拒绝将缺失旧档冒充为完整历史。

外部玩家命令若采用可靠验收，还需另有单调 `AdmissionCursor`，即“已经正式登记的
外部请求走到哪个连续编号”。`CommandAdmission` 记录 epoch、request/command identity、
实际 observed version、assigned tick 和规范载荷；MarketCommit 消费明确的 admission 区间。若宿主
...
无论选择哪项，NPC 可以只从当前 committed 根继续确定性推进；“恢复到相同版本”的保证只覆盖
durable 水位。已经回复“已持久验收入队”的命令不得位于 durable watermark 之后。
...
6. **热冷历史衔接完整：** 分别查询当前仍在追加的活动日 K、刚封存的日 K、十年前
   ...
   被悄悄冒充成已实现的能力。
7. **确定性：** 相同输入分别用 1、2、4、8 条线程运行，且在旧历史页已经缓存或尚未缓存
   两种情况下运行，均应得到同一股票、账户、计划和事件的业务根。定期用完整重算
   （`full hash`）核对增量摘要，确保没有漏掉某次权威状态改变。若结算或摘要检查
   失败，旧根、历史目录及已对外发布的事件都不能推进。
...
11. **只读和身份权限：** 发布新根、缓存淘汰或合并历史小段以后，用旧游标查询应仍
    ...
    当作自己的已知决策事实。多局并行运行时，不得共用可变状态、随机数、历史游标
    或账户权限，避免 A 局的一次操作影响 B 局。
...
6. **并行不改变市场结果。** 相同输入在不同线程数下应产生相同权威状态。同一股票
   保持适用价格—时间顺序，同一账户跨股票竞争现金有预先确定的仲裁，整个 tick
   要么完整提交要么完整失败。操作系统线程完成顺序不成为暗中的业务规则。
...
#### 11.2.3 真正同时输入的统一到达顺序怎样确定

**要决定的事情：** 当前有 `npc → player → plan_chain` 的来源类别顺序。移除这种
来源优先后，尚未被权威受理时间区分的同批输入，按什么稳定键先通过账户资源仲裁并
进入同一股票订单簿？已正式受理且有先后时间的订单不能事后洗牌。

**可选后果：** 按服务器正式验收序最容易审计，但网络延迟会影响先后；按固定交易者
或请求编号可稳定重放，但某些编号可能长期先得；仅对真正同时且尚未赋权威顺序的
请求做可重放置换，可减少固定编号偏向，却需要不可操纵的种子、输入集合和审计记录，
还可能与真实交易所按受理时间的语义不同。后者若采用，应明确登记为游戏简化，
不能拿它重排已受理同价订单。

**本文建议：** 先取消来源类别本身的优先权，再由产品明确选定权威受理键；若在线
联机优先考虑正式验收序，若离线批处理另有需求，应先核对适用官方价格—时间规则并
记录简化。这个决定会改变市场路径、同账户资金分配和重放结果，不能在实现中隐含。
...
3. **逐类替换整局复制。** 先让很少变化的静态资料和公司 Civil 资料共享，再把
   ...
   本步仍要保证旧版本可读、失败 tick 可整体丢弃、同一输入得到相同业务结果；
...
4. **把权威历史迁到分块保存。** 新增当日分时和活动日 K 先进入有界尾页，完成日 K
   ...
   与段损坏都要显式失败。
5. **迁移增量摘要与进程内回滚；可靠恢复另待批准。** 新增带格式版本的摘要树，
   迁移期保留较慢的全量算法作为对照工具：同一状态抽样完整重算，确认每个账户、
   计划、随机数和编号游标都被覆盖。验证失败 tick 不推进根或历史截止位置后，
   才移除生产 tick 中全量 JSON 哈希和整局 witness clone。若第 11.2.4 另行批准
...
7. **统一玩家、NPC 与计划的请求。** 三者都携带实际观察版本、交易者身份、命令
   身份及规范化内容。第 11.2.2、11.2.3 获得决定后，再实现旧页面请求政策和
   同批到达顺序；`source` 只作审计，不作价格相同订单的自动优先级。验证同一
...
所谓“优化完成”须以相同输入的业务状态不漂移为前提；只有第 11 节明确获准的语义
变更可以作为差异，并且必须在 ADR、测试和玩家可见说明中一并记录。
...
3. **每个 NPC、账户或股票各建一条常驻线程或 actor。** 若有 20,000 个 NPC，
   ...
   数再多也不能取消这些真实依赖。建议用有限的通用原生线程处理当前确实到期
   的批量计算，而不把业务身份变成线程身份。
...
旧观察请求、同批到达顺序、永久历史粒度、旧私有版本期限以及是否新增崩溃恢复，
...
3. 确定旧观察请求、同批到达顺序和旧私有版本查询期限，更新统一宿主协议、UI 提示
