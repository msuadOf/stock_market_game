...
- **状态：** proposed
- **日期：** 2026-09-23
- **决策者：** msuad（2026-09-24 已明确只在真实冲突与计划依赖处建立交易先后；其余冲突项仍待逐项确认）
...
- [`PlanBook`](../../packages/engine/src/plans/mod.rs#L128) 仍将终止计划留在权威存储中，序列化时
  ...
  存档、整局 shadow 与显式全量哈希仍处理终止历史。存档的百万计划上限只是校验门槛，
...
flowchart LR
    ...
    HUMAN --> EXTERNAL["外部命令入队<br/>记录投递身份与观察版本"]
    EXTERNAL --> FREEZE["Freeze n：固定本轮外部输入<br/>旧观察根及根计划资格"]
    R --> FREEZE
    FREEZE --> NPC
    R --> P0["P0 到期处理<br/>ExecutionBudget n"]
    NPC --> INGRESS["异步受理就绪请求<br/>玩家 / NPC / 计划同等地位"]
    FREEZE --> INGRESS
...
3. 玩家命令、NPC 意图和已就绪的计划命令同等地位。外部命令仍记录投递身份、观察版本和所属 tick；交易请求只在同股票、同账户实际资源争用处记录局部受理先后，不通过单一全局计数器决定优先级。`Freeze(n)` 只封住本轮可接收的**外部**玩家请求、观察根和根计划资格；之后才到达的外部请求进入后续 tick。某计划依赖自己的前一步结果时，后续命令仅在该结果出现后成为就绪请求，不占住其他已就绪请求的位置。同 tick 的 typed continuation 仍可在后续就绪轮处理；这不是允许封口后的新外部请求插入本轮。
...
后续子命令。后两者在本轮内部成为就绪请求；只有触及同一账户资源或同一股票的请求
...
用户已在 2026-09-24 明确：玩家、NPC 和计划提出的交易请求地位相同。先后只在真实冲突处裁决：同股按适用的价格、时间规则，同账户争用同一笔现金或股份时按该账户收到请求的顺序，同一计划后续动作仅在确实需要前一步结果时等待。两个账户交易不同股票且互不争用资源时，无须比较谁先谁后。事件编号、输出排序和存档记录可以稳定，但不能反过来成为交易优先级；也不得为分配全局递增号使所有请求经过单一关口。当前 `npc → player → plan_chain` 类序以及账户、计划编号遍历造成的隐含优先级均待拆除。
每个 tick 对外部新到请求有明确封口边界；封口后到达的外部请求进入后续 tick。
计划链依赖前一 typed 执行结果的后续命令，只有该结果产生后才成为就绪请求，不能为了“先完整处理一条链”压住其他请求。账户 A 和 B 各有“撤旧→下新”时，两条已就绪撤单可以同批处理；撤销不同旧单无需仅因同股而分出交易优先级，撤单按目标订单、所有权、交易阶段及实际发生的本股冲突处理。A 的新单只等 A 撤单结果，不必等 B 的整条链。链内后续命令的同 tick 可见性仍按 ADR-0017 已接受的 continuation 边界执行。审计记录应能解释局部冲突如何裁决；不能为了重放同一次运行，把互不冲突的请求也强制排进同一个序列。当前实现仍在 `step()` 内先生成 NPC 候选，再拼接玩家队列和计划命令；计划链已开始按不同账户同批处理。P3 已不再用全局密封序在同一就绪批的共享容量申请者之间挑赢家；P4 的同股处理仍受全局密封序影响。这些编号不能解释为已实现的局部受理规则。
...
并另行记录本 tick 中足以复现局部冲突的受理事实与依赖关系。若宿主
...
**2026-09-24 决定：** 玩家、NPC 和计划异步提出请求，来源没有优先级。只有同股委托、同账户实际资金或股份争用、同一计划对前一步结果的依赖才建立交易先后；同股按适用价格时间规则，同账户按本账户收到请求的顺序，计划只等待真正依赖的结果。互不冲突的账户和股票可以同时计算，无须全局受理总序。稳定事件与存档身份只负责记录和重放，不得充当交易优先级，也不得用全局递增号把所有请求排进一个关口。每个 tick 统一完成校验、撮合和状态发布。现行 `npc → player → plan_chain` 类序、按账户或计划编号遍历带来的隐含优先级仍待迁移；在完成前不得称三类请求已经平权。
