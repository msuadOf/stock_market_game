...
**Claude 倾向**（待 msuad 确认）：参考真实市场微观结构文献（如 Kyle 1985、噪声交易者模型、Zero-Intelligence trader）为每类 NPC 设计可配置的策略 + 参数（风险偏好/换手率/订单大小分布等），全部经 `GameConfig` 暴露为可调旋钮。**AI 不擅自定具体策略，先研究后上报。**

> ⏳ 待 msuad 与 Claude 共同定夺。→ 解决后产出 ADR + 落到 `account`/`market` 模块的策略 trait 接口。
